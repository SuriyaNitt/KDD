...
def read_and_normalize_train_data_old(img_rows, img_cols, color_type=1):
...
def run_cross_validation(nfolds=10):
    ...
    #test_data, test_id = train_data, train_id  # read_and_normalize_test_data(img_rows, img_cols, color_type_global)
    ...
    for train_drivers, test_drivers in kf:
        ...
        y_train, train_index = copy_selected_drivers(train_target, driver_id, unique_list_train)
        ...
        if not os.path.isfile(kfold_weights_path) or restore_from_last_checkpoint == 0:
            for epoch in range(nb_epoch):
                ...
                nb_batches = math.ceil(len(y_train)/float(batch_size))
                bar = progressbar.ProgressBar(maxval=nb_batches, \
                                              widgets=[progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage()])
                bar.start()
                count = 0

                while items_done < len(y_train):
                    print('Batch {} of {}'.format(count, nb_batches))
                    if (len(y_train) - items_done) < batch_size:
                        batch_list = epoch_list[items_done:]
                        items_done = len(y_train)
                    else:
